...
typedef struct {
  ...
  uint32_t page_num;
  uint32_t cell_num;
  bool end_of_table; // one past last element
} Cursor;
...
uint32_t get_children_max_key(Pager* pager, void* node) {
  switch (get_node_type(node)) {
    case NODE_INTERNAL: {
...
// finds the position of key, or if not present
// finds position where it should be inserted.
...
void create_new_root(Table* table, uint32_t right_child_page_num) {
  // data in root copied to new page, becomes left child.
  // Old node becomes root and points to left and right.
  ...
  *internal_node_child(root, 0) = left_child_page_num;
  ...
  uint32_t left_child_max_key = get_children_max_key(table->pager, left_child);
...
void internal_node_split_and_insert(Table* table, uint32_t node_page_num, uint32_t child_page_num) {
  ...
  const int NUM_CHILDREN = INTERNAL_NODE_MAX_CELLS + 1;
  const int NEW_NODE_COUNT = NUM_CHILDREN / 2;
  ...
  if (child_max_key > get_children_max_key(table->pager, right_child)) {
    ...
    *internal_node_key(new_node, NEW_NODE_COUNT - 1) = get_children_max_key(table->pager, right_child);
  } else {
    *internal_node_right_child(new_node) = *internal_node_right_child(node);
  }

  for (int32_t i = children_remaining - 1; i >= 0; i--) {
...
void internal_node_insert(Table* table, uint32_t parent_page_num, uint32_t child_page_num) {
  ...
  *internal_node_num_keys(parent) = original_num_keys + 1;
  ...
  if (child_max_key > get_children_max_key(table->pager, right_child)) {
    ...
    *internal_node_key(parent, original_num_keys) = get_children_max_key(table->pager, right_child);
...
void leaf_node_split_and_insert(Cursor* cursor, uint32_t key, Row* value) {
  // create new node and move half cells over.
  // Insert new value
  // Update (or create new) parent
  ...
  // new node gets a pointer to the _old_ next node
